...
def build_reports(
    ...
    for group_name, cols in groups.items():
        ...
        report_out = report_df[["label", "detail", "value", "pct"]].rename(
            columns={"label": "Etiqueta", "detail": "Detalle", "value": "N", "pct": "%"}
        )
    ...
    combined_out = combined_df[["group_label", "label", "detail", "value", "pct"]].rename(
        columns={"group_label": "Variable", "label": "Etiqueta", "detail": "Detalle", "value": "N", "pct": "%"}
    )
    combined_out.to_csv(combined_csv, index=False)

    combined_xlsx = RESULTS_DIR / f"{output_prefix}consolidado.xlsx"
    with pd.ExcelWriter(combined_xlsx, engine="openpyxl") as writer:
        combined_out.to_excel(writer, index=False, sheet_name="Consolidado")
        for rep in reports:
            sheet_name = _safe_filename(rep["group_label"])[:31]
            sheet_df = pd.DataFrame(rep["rows"])[["label", "detail", "value", "pct"]].rename(
                columns={"label": "Etiqueta", "detail": "Detalle", "value": "N", "pct": "%"}
            )
    ...
    for rep in reports:
        ...
        table_df.columns = ["Etiqueta", "Detalle", "N", "%"]
